	...
	public void handle(String target, Request baseRequest,
		...
		for (Endpoint endpoint : this.endpoints) {
			...
			if (url.equals(target)) {
				...
				if (preprocessors != null) {
					for (String classname : preprocessors.getPreprocessor()) {
						try {
							...
							paramTable = (Hashtable<String, String>) m.invoke(
									o, paramTable);
				...
				boolean isSelect = QueryFactory.create(
						sources.getSource().get(0).getQuery()).isSelectType();
				if (isSelect == true) {
					// all queries have to be SELECT queries
					for (int i = 1; i < sources.getSource().size(); i++) {
						if (QueryFactory.create(
